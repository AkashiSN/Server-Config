...
IAM ポリシーはオブジェクトストレージとして必要な action のみを許可し、`allowed_ips` (`/32` の list) からのリクエストに限定する。本リポジトリでは `module.juicefs_s3` (purpose=`juicefs`) として JuiceFS のオブジェクトストレージ用に呼び出している。
...
- バージョニングを有効化しているため、JuiceFS によって上書き / 削除されたオブジェクトは旧バージョンとして一時的に残るが、ライフサイクルルールにより 7 日後に自動削除される (誤削除に対するリカバリ猶予を確保しつつコスト増を抑制)。
- 現行バージョンを期限切れにするルールは入れていないため、JuiceFS の現行データが lifecycle で消えることはない。
