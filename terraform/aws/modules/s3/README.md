...
IAM ポリシーはオブジェクトストレージとして必要な action のみを許可し、`allowed_ips` (`/32` の list) からのリクエストに限定する。本リポジトリでは以下 2 用途で呼び出している:

- `module.juicefs_s3` (purpose=`juicefs`) — JuiceFS のオブジェクトストレージ
- `module.postgres_backup_s3` (purpose=`postgres-backup`) — WAL-G による Postgres バックアップ

## リソース構成

| リソース | 説明 |
| --- | --- |
| `aws_s3_bucket.this` | バケット名 `${project}-${purpose}` |
...
| `aws_iam_user.this` | IAM ユーザ `${project}_${purpose}` |
| `aws_iam_access_key.this` | 上記ユーザの access key / secret key |
| `aws_iam_user_policy.this` | `${project}_${purpose}` — 当該バケットに対するオブジェクトストレージ操作のみ許可 |
...
| `allowed_ips` | list(string) | IAM ユーザのアクセスを許可する送信元 IPv4 CIDR の list (`x.x.x.x/32` 形式)。複数ノードから許可する場合はノード分の `/32` を全て渡す |
...
| `noncurrent_days` | number | 非現行バージョンの保持日数 (lifecycle expiration)。デフォルト 7。WAL-G の retention (`wal-g delete retain FULL N`) は object を素直に DELETE するため、versioning 上は noncurrent 化される。本値が小さいと外部 retention ツールの保持期間より先に hard delete されてしまうため、WAL-G 用バケットでは長めに設定する (例: 週次フル × 4 世代なら 35 日) |
...
すべての statement に `aws:SourceIp` = `var.allowed_ips` の `IpAddress` 条件が付与される。
...
| JuiceFS 操作 | 必要な action |
| --- | --- |
| object list / metadata 同期 | `s3:ListBucket` |
| object 取得 (`GET`) | `s3:GetObject` |
| object 書き込み (小オブジェクト) | `s3:PutObject` |
| object 書き込み (マルチパート) | `s3:PutObject` + `s3:AbortMultipartUpload` + `s3:ListMultipartUploadParts` + `s3:ListBucketMultipartUploads` |
| object 削除 (gc / rmr) | `s3:DeleteObject` |
| region 判定 | `s3:GetBucketLocation` |

SSE-S3 (AES256) のため KMS 権限は不要。バケットは Terraform 側で作成するため `s3:CreateBucket` も不要。バケットでバージョニングを有効化しているが、JuiceFS はバージョン ID を意識せず通常の PUT / GET / DELETE のみで動作するため、`s3:GetBucketVersioning` や `s3:*ObjectVersion` 系の追加権限は不要。
...
- バージョニングを有効化しているため、上書き / 削除されたオブジェクトは旧バージョンとして一時的に残るが、ライフサイクルルールにより `noncurrent_days` (デフォルト 7) で自動削除される (誤削除に対するリカバリ猶予を確保しつつコスト増を抑制)。
- 現行バージョンを期限切れにするルールは入れていないため、現行データが lifecycle で消えることはない。
- Secret Access Key は `terraform output -raw <module 名>_iam_secret_access_key` で取得する。
- 本リポジトリでの `noncurrent_days` 使い分け: `module.juicefs_s3` は default 7 (JuiceFS は object を上書きしてもメタ DB 側で参照管理される) / `module.postgres_backup_s3` は **35** (WAL-G の `wal-g delete retain FULL 4` で削除した世代が、対応する WAL も含めて 35 日間ロールバック可能であることを保証)。
