...
(
	...
	kubectl create secret generic ${PRODUCT}-daemon-certs -n buildkitd --dry-run=client -o yaml --from-file=./daemon >${PRODUCT}-daemon-certs.yml
	kubectl create secret generic ${PRODUCT}-client-certs -n buildkitd --dry-run=client -o yaml --from-file=./client >${PRODUCT}-client-certs.yml
)
